 ...
 * @author Rais
 */
public class Couleur_op {
    private static Couleur_op instance;   
public Couleur_op(){}
public static Couleur_op getInstance(){
if(instance==null){
    instance=new Couleur_op();
}
return instance;
}
 ...
 try {
          Connection conn = jdbc.Getconnection();
             ...
             String sql;
                  ...
                  sql = "SELECT count(valeur) as nb_v FROM couleur ";
...
public boolean  CouleurExist(String valeur) throws Exception {
         ImageIcon   img = new ImageIcon("Images/erreur.GIF");
 boolean a=false;
        try {
             Connection conn = jdbc.Getconnection();
              String sql = "select * FROM couleur where valeur=?";
        ...
        while (rs.next()) {
            ...
            {  JOptionPane.showMessageDialog(null, " Cette couleur existe deja  ", " ", JOptionPane.WARNING_MESSAGE, img);
            a=true;
            }
        }
        rs.close();
...
public  void inserer_couleur(String valeur )throws Exception{
        try {
            Connection conn = jdbc.Getconnection();
              String sql = "insert into couleur (valeur) values (?) ";
              PreparedStatement pr=conn.prepareStatement(sql);
              pr.setString(1,valeur);
              pr.executeUpdate();
           // ResultSet rs = stmt.executeQuery(sql);
...
public  void Charger_couleur( JComboBox categorie ){ 
...
categorie.removeAllItems();
 ...
 try {
           Connection conn = jdbc.Getconnection();
             Statement stmt = conn.createStatement();
              String sql = "SELECT * FROM couleur  order by valeur asc";
            ResultSet rs = stmt.executeQuery(sql);
            categorie.addItem(" ");
        while (rs.next()) {
  ...
  public  void sup_Couleur(String  marque)throws Exception{

        try {
 Connection conn = jdbc.Getconnection();

              String sql = "delete  from couleur where valeur=?";
           PreparedStatement pr=conn.prepareStatement(sql);
