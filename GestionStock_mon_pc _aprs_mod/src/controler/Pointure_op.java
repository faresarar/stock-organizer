 ...
 * @author Rais
 */
public class Pointure_op {
    private static Pointure_op instance;   
public Pointure_op(){}
public static Pointure_op getInstance(){
if(instance==null){
    instance=new Pointure_op();
}
return instance;
}
 ...
 try {
            Connection conn = jdbc.Getconnection();
             ...
             String sql;
                  ...
                  sql = "SELECT count(valeur) as nb_v FROM pointure ";
 ...
 try {
           Connection conn = jdbc.Getconnection();
             ...
             String sql;
                  ...
                  sql = "SELECT valeur FROM pointure order by valeur asc";
...
public boolean PointureExist(String valeur) throws Exception {
         ImageIcon   img = new ImageIcon("Images/erreur.GIF");
 boolean a=false;
        try {
           Connection conn = jdbc.Getconnection();
              String sql = "select * FROM pointure where valeur=?";
...
public String  getidPointure(String valeur) throws Exception {
         
 String id=null;
        try {
              Connection conn = jdbc.Getconnection();
              String sql = "select id FROM pointure where valeur=?";
           PreparedStatement pr=conn.prepareStatement(sql);
              pr.setString(1,valeur);
...
public  void inserer_pointure(String valeur )throws Exception{
        try {
             Connection conn = jdbc.Getconnection();
             String sql = "insert into pointure (valeur) values (?) ";
               PreparedStatement pr=conn.prepareStatement(sql);
               pr.setString(1, valeur);
             //  pr.setString(2, id);
               pr.executeUpdate();
...
public String GetNextReferencePointure() throws Exception{
    String ref="C";
    int max=0;
          Connection conn = jdbc.Getconnection();
            Statement stmt = conn.createStatement();
             String sql = "select max(id) as maxrow from pointure ";
            ResultSet rs = stmt.executeQuery(sql);
        while (rs.next()) {
...
public  void Charger_pointure( JComboBox categorie ){ 
...
categorie.removeAllItems();
 ...
 try {
             Connection conn = jdbc.Getconnection();
             Statement stmt = conn.createStatement();
              String sql = "SELECT * FROM pointure  order by valeur asc";
            ResultSet rs = stmt.executeQuery(sql);
            categorie.addItem(" ");
        while (rs.next()) {
  ...
  public  void sup_pointure(String  produit)throws Exception{

        try {

             Connection conn = jdbc.Getconnection();

              String sql = "delete  from pointure where valeur=?";
              PreparedStatement pr=conn.prepareStatement(sql);
